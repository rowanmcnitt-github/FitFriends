  ...
  if(liftDats != null)
    {
      ...
      charSizeMod = (liftDats.dat[0] + liftDats.dat[1] + liftDats.dat[2]) / 220 * 5;
...
function attemptRegister()
{
  if(!logging)
    ...
    customizeCharacter = new userData(username, color(random(0,255), random(0,255), random(0,255)),color(random(0,255), random(0,255), random(0,255)), int(random(0,faceList.length)), "");
...
function syncData(fakeName)
{
  let syncUser = new userData(fakeName, color(255,100,100), color(100,200,10), 0, "");
...
function draw() 
{
  ...
  if(logStatus == 2)
    ...
    if(!comparing)
    {
      ...
      drawCharacter(currentUser.shirtColor,currentUser.skinColor, characterSize, characterRot + newRot, faceList[currentUser.faceType], armRot + 0.2*sin(millis()/2000),armRot -0.2*sin(millis()/2000), userLiftData);
        pop();
      }
...
function parseCColor(Val)
  {
    ...
    print(goodVals);
    return color(int(goodVals[0]), int(goodVals[1]), int(goodVals[2]));
  }
